...
export default function Login() {
    const { login, setActiveSession } = useAuth();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // If user has an active session, go directly to charging page
            if (data.user?.role !== 'admin') {
                try {
                    const sessionRes = await api.get('/session/active');
                    const session = sessionRes.data?.session;
                    if (session?.stationId) {
                        setActiveSession({ stationId: session.stationId });
                        navigate(`/user/charging/${session.stationId}`, { replace: true });
                        return;
                    }
                } catch {
